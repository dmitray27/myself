...
- Перед текстом в каждом блоке идёт заголовок `AFSK_HDR_LEN = 3` байта (`seq`, `block_no`, `total`, см. `afsk_block_hdr_t`); RX собирает сообщение по номерам блоков и отдаёт его только полным, иначе печатает `INCOMPLETE MESSAGE` и шлёт `System:`-уведомление. Прошивки с заголовком и без несовместимы по эфиру — TX и RX прошивать вместе.
