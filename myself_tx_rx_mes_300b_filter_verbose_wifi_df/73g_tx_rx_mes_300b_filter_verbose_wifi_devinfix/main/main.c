...
static void rx_task(void *pvParameters)
{
    ...
    char *assembly = (char *)malloc(RX_ASSEMBLY_MAX + 1);
    if (!assembly) {
    ...
    int assembly_len = 0;
    uint32_t assembly_blocks = 0;
    uint32_t assembly_dropped = 0;
    ...
    while (1) {
        if (assembling &&
            ...
            assembly[assembly_len] = '\0';
            printf("\n########################################\n");
            printf("[RX] FULL MESSAGE: %d bytes in %" PRIu32 " blocks\n",
                   assembly_len, assembly_blocks);
            if (assembly_dropped > 0) {
        ...
        for (int i = 0; i < samples; i += RX_SLOTS_PER_FRAME) {
            ...
            if (afsk_decoder_process_sample(&decoder, sample, &message)) {
                ...
                if (message.crc_valid) {
                    ...
                    if (n > 0) {
                        memcpy(assembly + assembly_len, message.text, n);
                        assembly_len += n;
                    }
                    if (n < message.length) {
