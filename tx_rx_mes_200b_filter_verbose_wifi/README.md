...
| Preamble | 640 бит |
| Заголовок блока | 3 байта: `seq` (счётчик сообщений, 0–255), `block_no` (с 0), `total` |
| Полезная нагрузка блока | 50 байт текста (кадр = 3 + до 50 + CRC) |
| CRC | CRC-8 (poly 0x07, init 0x00) по заголовку и тексту |
| PTT lead | 300 мс |
| PTT tail | 100 мс |
...
- Сообщение с эфира, в котором не хватает хотя бы одного блока (CRC FAIL или блок вообще не принят), в чат не рассылается — вместо него всем клиентам уходит `System:Сообщение из эфира принято не полностью (получено блоков: X из N)`.
...
- Каждый блок начинается с 3-байтного заголовка `seq / block_no / total`
  (`afsk_block_hdr_t`, `AFSK_HDR_LEN`) и целиком защищён CRC-8. `seq` — счётчик
  сообщений TX (по модулю 256), поэтому сообщение — не более `AFSK_MAX_BLOCKS = 255` блоков.
- RX раскладывает блоки по номеру в слоты текущего `seq`. Как только пришли все `total`
  блоков, сообщение отдаётся сразу, не дожидаясь тишины. Пауза дольше `MESSAGE_IDLE_MS`
  (1.5 × `BLOCK_TIME_MS`) или блок с другим `seq` закрывает сборку как есть; буфер
  `RX_ASSEMBLY_MAX = 8192` байт, дубликаты блоков игнорируются.
- Собранное сообщение уходит в Wi-Fi как `Remote:r<n>:<текст>` ровно с длиной сборки
  (без `strlen`, чтобы встроенный `\0` не обрезал текст). Если блоков меньше `total`
  (CRC FAIL, пропущенная преамбула, обрыв кадра — RX это не различает, он видит пустой
  слот), сообщение не рассылается — только `System:`-уведомление о потере, а в UART
  вместо `FULL MESSAGE` печатается `INCOMPLETE MESSAGE: seq S, X of N blocks received`
  и `WARNING: K block(s) lost`.
- Обе стороны печатают CRC32 (zlib-совместимый, `esp_rom_crc32_le`) всего сообщения:
  TX — в `Message: N bytes, Blocks: B, CRC32: XXXXXXXX, Seq: S`, RX — в `FULL MESSAGE: ... CRC32: XXXXXXXX, seq S`.
...
- Время передачи одного блока 50 байт: примерно 6.9 с (с учётом заголовка, preamble, PTT и межблочной паузы).
- Прошивки с заголовком блока и без него несовместимы по эфиру — TX и RX платы прошиваются вместе.
