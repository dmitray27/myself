...
System:<текст>              # служебное уведомление только этому клиенту
```
...
- Собранное сообщение уходит в Wi-Fi как `Remote:r<n>:<текст>` ровно с длиной сборки
  (без `strlen`, чтобы встроенный `\0` не обрезал текст).
...
|---|---|---|---|
| Заголовок `FULL MESSAGE` (без тела) | `rx_task` | ~5 мс на сообщение | всегда |
